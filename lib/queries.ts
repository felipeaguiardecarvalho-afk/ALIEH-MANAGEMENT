...
function mapProductRow(row: Record<string, unknown>): Product {
  return {
    ...
    productImagePath: (row.product_image_path as string | null) ?? null,
...
export async function getProductById(
  ...
  if (!hasDatabaseUrl || !Number.isFinite(id) || id <= 0) return null;

  try {
    const rows = await db()`
      SELECT id, name, sku, registered_date, product_enter_code, cost, price,
             pricing_locked, stock, frame_color, lens_color, style, palette, gender,
             product_image_path, created_at
      FROM products
      WHERE tenant_id = ${tenantId}
        AND id = ${id}
        AND deleted_at IS NULL
    ...
    logFallback("product by id", error);
    return null;
  }
}
...
export async function getUatRecords(tenantId = getTenantId()): Promise<UatRecord[]> {
  if (!hasDatabaseUrl) return [];

  try {
    const rows = await db()`
      SELECT DISTINCT ON (test_id)
             test_id, status, notes, result_recorded_at, updated_at,
             recorded_by_username, recorded_by_role
      FROM uat_manual_checklist
      WHERE tenant_id = ${tenantId}
      ORDER BY test_id, updated_at DESC;
    `;
    ...
    return rows.map((row) => ({
      ...
      resultRecordedAt: (row.result_recorded_at as string | null) ?? null,
      updatedAt: (row.updated_at as string | null) ?? null,
      recordedByUsername: (row.recorded_by_username as string | null) ?? null,
      recordedByRole: (row.recorded_by_role as string | null) ?? null,
    }));
  } catch (error) {
    logFallback("uat records", error);
    return [];
  }
}
